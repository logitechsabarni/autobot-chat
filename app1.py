...
def col_(i): return PALETTE[i%len(PALETTE)]
...
def make_heatmap(ml):
    ...
    fig=go.Figure(go.Heatmap(z=z,x=DIM_LABELS,y=y,
    ...
    fig.update_layout(**pd_(),height=max(250,len(ml)*55+80),
...
def make_bar(ml,key,label):
    fig=go.Figure(go.Bar(
        x=[m["name"] for m in ml],y=[m.get(key,0) for m in ml],
        text=[str(m.get(key,0)) for m in ml],textposition="outside",
    ...
    fig.update_layout(**pd_(),height=280,yaxis=dict(range=[0,115],**gs_()),
...
def make_bubble(ml):
    ...
    for i,m in enumerate(ml):
        fig.add_trace(go.Scatter(
            x=[m.get("price_in",0)],y=[m.get("overall",0)],
            mode="markers+text",name=m["name"],text=[m["name"]],textposition="top center",
            marker=dict(size=max(m.get("context",10000)/15000,12),color=col_(i),opacity=0.85,
                        line=dict(color="#eeeef8",width=1)),
            textfont=dict(color="#eeeef8",family="JetBrains Mono",size=10)))
    fig.update_layout(**pd_(),height=380,showlegend=False,
...
def make_timeline(ml):
    ...
    for i,m in enumerate(rows):
        fig.add_trace(go.Scatter(
            x=[m["release_date"]],y=[m.get("overall",0)],
            mode="markers+text",name=m["name"],text=[m["name"]],textposition="top center",
            marker=dict(size=14,color=col_(i),symbol="diamond",line=dict(color="#eeeef8",width=1)),
            textfont=dict(color="#eeeef8",family="JetBrains Mono",size=10)))
    fig.update_layout(**pd_(),height=320,showlegend=False,
        xaxis=dict(title="Release Date",**gs_(),titlefont=dict(color="#5a5a80")),
        yaxis=dict(title="Overall Score",**gs_(),titlefont=dict(color="#5a5a80")))
    return fig

def score_ring_html(score,color,size=80):
    r=(size-10)/2; circ=2*math.pi*r; dash=circ*(score/100)
    return f'<svg width="{size}" height="{size}" class="ring-svg"><circle cx="{size/2}" cy="{size/2}" r="{r}" fill="none" stroke="#1f1f38" stroke-width="7"/><circle cx="{size/2}" cy="{size/2}" r="{r}" fill="none" stroke="{color}" stroke-width="7" stroke-dasharray="{dash:.1f} {circ:.1f}" stroke-linecap="round"/></svg>'

# ═══════════════════════════════════════════════════════
# SESSION STATE
# ═══════════════════════════════════════════════════════
for k,v in {"models":[],"prompt":"","responses":{},"weights":{d:1.0 for d in DIMS}}.items():
...
with st.sidebar:
    ...
    if cp1.button("Load Presets",use_container_width=True):
        ...
        for p in PRESET_MODELS:
            p2=p.copy(); p2["overall"]=compute_overall(p2); ps.append(p2)
        st.session_state.models=ps; st.rerun()
    if cp2.button("Clear All",use_container_width=True):
        st.session_state.models=[]; st.session_state.responses={}; st.rerun()
        ...
        with st.form("add_form",clear_on_submit=True):
            ...
            if st.form_submit_button("Add to Arena ➜",use_container_width=True):
                if not name.strip():
                    ...
                    m=dict(name=name.strip(),provider=provider,version=version,category=category,
    ...
    if st.session_state.models:
        with st.expander("⚖️ Custom Score Weights"):
            ...
            for d,label in zip(DIMS,DIM_LABELS):
                st.session_state.weights[d]=st.slider(label,0.0,3.0,st.session_state.weights.get(d,1.0),0.1,key=f"w_{d}")
            if st.button("Apply Weights",use_container_width=True):
                w=st.session_state.weights; total=sum(w.values()) or 1
...
<div class="glass-card" style="border-color:#7c6fff;margin-bottom:1.5rem;background:linear-gradient(135deg,rgba(124,111,255,.07),rgba(0,212,170,.04))">
  <div style="display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap">
    <div>
      ...
      <div class="tag-row">
        ...
        <span class="glow-chip chip-{"green" if winner.get("open_source") else "yellow"}">{winner.get("license","")}</span>
      </div>
    </div>
  ...
  {"<div style='margin-top:.8rem;font-family:JetBrains Mono,monospace;font-size:.78rem;color:#8888b0;border-top:1px solid #1f1f38;padding-top:.8rem'>"+winner.get("benchmark","")+"</div>" if winner.get("benchmark") else ""}
</div>""", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    for rank,m in enumerate(sorted_models):
        color=col_(rank); pct=min(int(m["overall"]),100)
        ...
        if m.get("open_source"): badge_html+='<span class="glow-chip chip-blue" style="font-size:.65rem">OSS</span>'
        ...
        dim_mini="".join([
            f'<div style="flex:1;min-width:60px"><div style="font-family:JetBrains Mono,monospace;font-size:.6rem;color:#5a5a80;margin-bottom:.2rem">{l[:5].upper()}</div>'
        ...
        st.markdown(f"""
        <div class="glass-card" style="margin-bottom:.8rem;padding:1rem 1.4rem;border-color:{"#7c6fff" if rank==0 else "#1f1f38"}">
    ...
    if bench_data:
        ...
        for bench,bc in [("mmlu","#7c6fff"),("gsm8k","#00d4aa"),("humaneval","#ffb340")]:
            fig_b.add_trace(go.Bar(name=bench.upper(),x=[m["name"] for m in bench_data],
                y=[m.get(bench,0) for m in bench_data],marker_color=bc,
                text=[f"{m.get(bench,0):.1f}" for m in bench_data],textposition="outside",
                textfont=dict(color="#eeeef8",size=10)))
        fig_b.update_layout(barmode="group",**pd_(),height=300,
            yaxis=dict(range=[0,115],**gs_()),xaxis=dict(tickfont=dict(color="#eeeef8",size=10)),
...
with tabs[1]:
    ...
    with cr:
        ...
        sel_r=st.multiselect("Models",[m["name"] for m in models],
            default=[m["name"] for m in models[:min(4,len(models))]],key="radar_sel")
        if [m for m in models if m["name"] in sel_r]:
            st.plotly_chart(make_radar([m for m in models if m["name"] in sel_r]),
...
with tabs[2]:
    ...
    for row_i in range(2):
        ...
        for col_i,(d,l) in enumerate([(DIMS[row_i*4+j],DIM_LABELS[row_i*4+j]) for j in range(4) if row_i*4+j<len(DIMS)]):
            with cols_row[col_i]:
    ...
    if len(models)>=2:
        ...
        fig_par=go.Figure(go.Parcoords(
            line=dict(color=df_par["overall"],colorscale=[[0,"#1f1f38"],[0.5,"#7c6fff"],[1,"#00d4aa"]],showscale=True),
            dimensions=[dict(label=l,values=df_par[d]) for d,l in zip(DIMS,DIM_LABELS)]+[dict(label="Overall",values=df_par["overall"])]))
...
with tabs[3]:
    ...
    with cl2:
        ...
        for i,m in enumerate(sorted(models,key=lambda m:m.get("cost_eff",0),reverse=True)):
            pct=m.get("cost_eff",0); c=col_(i)
            st.markdown(f"""<div style="margin-bottom:.7rem"><div style="display:flex;justify-content:space-between;margin-bottom:.25rem"><span style="font-family:'JetBrains Mono',monospace;font-size:.78rem">{m["name"]}</span><span style="font-family:'Outfit',sans-serif;font-weight:800;color:{c}">{pct}</span></div><div class="rank-bar"><div class="rank-bar-fill" style="width:{pct}%;background:{c}"></div></div></div>""",unsafe_allow_html=True)
    ...
    fig_p=go.Figure()
    fig_p.add_trace(go.Bar(name="Input $/1M",x=[m["name"] for m in sp],y=[m.get("price_in",0) for m in sp],
        marker_color="#7c6fff",text=[f'${m.get("price_in",0):.2f}' for m in sp],textposition="outside",textfont=dict(color="#eeeef8",size=10)))
    fig_p.add_trace(go.Bar(name="Output $/1M",x=[m["name"] for m in sp],y=[m.get("price_out",0) for m in sp],
        marker_color="#00d4aa",text=[f'${m.get("price_out",0):.2f}' for m in sp],textposition="outside",textfont=dict(color="#eeeef8",size=10)))
    fig_p.update_layout(barmode="group",**pd_(),height=300,yaxis=dict(**gs_()),
        xaxis=dict(tickfont=dict(color="#eeeef8",size=10)),legend=dict(bgcolor="rgba(0,0,0,0)",font=dict(color="#eeeef8")))
    ...
    calc_cols=st.columns(min(5,len(models)))
    for i,m in enumerate(sorted_models[:len(calc_cols)]):
        per_call=(in_tok/1_000_000*m.get("price_in",0))+(out_tok/1_000_000*m.get("price_out",0))
        total=per_call*calls*mult; c=col_(i)
        with calc_cols[i%len(calc_cols)]:
            st.markdown(f"""<div class="glass-card" style="text-align:center;border-color:{c};padding:1rem"><div style="font-family:'Outfit',sans-serif;font-weight:800;color:{c};font-size:.9rem">{m["name"]}</div><div style="font-family:'Outfit',sans-serif;font-weight:900;font-size:1.9rem;color:{c};line-height:1.1;margin:.3rem 0">${total:.2f}</div><div style="font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#5a5a80">per {days_sel.lower()}</div><div style="font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#5a5a80;margin-top:.3rem">${per_call*calls:.2f}/day · ${per_call*calls*365:.0f}/year</div></div>""",unsafe_allow_html=True)

# ── TAB 4: RESPONSES ────────────────────────────────
with tabs[4]:
    st.markdown('<p class="section-label">SHARED PROMPT</p>',unsafe_allow_html=True)
    prompt=st.text_area("Prompt used to test all models",value=st.session_state.prompt,placeholder="What is the speed of light?",height=80)
    st.session_state.prompt=prompt
    if prompt.strip():
        st.markdown(f"""<div class="glass-card" style="margin-bottom:1rem;border-color:#7c6fff"><div style="font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#5a5a80;margin-bottom:.4rem;letter-spacing:.1em">PROMPT</div><div style="font-family:'Outfit',sans-serif;font-size:.95rem;color:#c8c8e8">{prompt}</div></div>""",unsafe_allow_html=True)
    ...
    with st.expander("➕ Log a Response",expanded=False):
        ...
        if st.button("Log Response",key="log_btn"):
            ...
            else: st.warning("Paste a response first.")
    ...
    if resp_models:
        ...
        for i,m in enumerate(resp_models):
            ...
            with cols_r[i%2]:
                st.markdown(f"""<div class="glass-card" style="border-color:{c};margin-bottom:1rem"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.7rem"><span style="font-family:'Outfit',sans-serif;font-weight:800;color:{c}">{m["name"]}</span><span style="font-family:'JetBrains Mono',monospace;font-size:.68rem;color:#5a5a80">{m.get("provider","")}</span></div><div style="background:var(--s1);border-radius:10px;padding:.9rem;font-family:'JetBrains Mono',monospace;font-size:.78rem;color:#c8c8e8;line-height:1.7;white-space:pre-wrap;max-height:200px;overflow:auto">{m["public_resp"]}</div>{"<div style='margin-top:.6rem;font-family:JetBrains Mono,monospace;font-size:.7rem;color:#5a5a80'>" + m.get("benchmark","") + "</div>" if m.get("benchmark") else ""}{"<div style='margin-top:.4rem;font-family:JetBrains Mono,monospace;font-size:.7rem;color:#8888b0'>" + m.get("notes","") + "</div>" if m.get("notes") else ""}</div>""",unsafe_allow_html=True)
            ...
            with st.expander(f"📌 {prompt_key[:80]}"):
                ...
                for j,e in enumerate(entries):
                    ...
                    with cols_e[j%len(cols_e)]:
                        st.markdown(f"""<div class="glass-card" style="border-color:{c}"><div style="display:flex;justify-content:space-between;margin-bottom:.5rem"><span style="font-family:'Outfit',sans-serif;font-weight:800;color:{c}">{e["model"]}</span><span style="font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#5a5a80">{e["ts"]}</span></div><div style="background:var(--s1);border-radius:8px;padding:.7rem;font-family:'JetBrains Mono',monospace;font-size:.75rem;color:#c8c8e8;line-height:1.6;white-space:pre-wrap;max-height:140px;overflow:auto">{e["response"][:400]}{"…" if len(e["response"])>400 else ""}</div><div style="display:flex;gap:.3rem;margin-top:.5rem;flex-wrap:wrap"><span class="glow-chip chip-purple">⭐ {e["rating"]}/10</span><span class="glow-chip chip-green">⚡ {e["latency"]}ms</span><span class="glow-chip chip-yellow">🪙 {e["tokens"]} tok</span></div>{"<div style='font-family:JetBrains Mono,monospace;font-size:.68rem;color:#5a5a80;margin-top:.4rem'>"+e["notes"]+"</div>" if e.get("notes") else ""}</div>""",unsafe_allow_html=True)
...
with tabs[5]:
    if len(models)<2:
        ...
        m2n=hh2.selectbox("Model B",[m["name"] for m in models if m["name"]!=m1n],key="hhb")
        ma=next(m for m in models if m["name"]==m1n)
        mb=next(m for m in models if m["name"]==m2n)
        ca=col_(models.index(ma)); cb=col_(models.index(mb))
        wins_a=wins_b=0
        rows_hh=[]
        ...
        for label,va,vb in rows_hh:
            ...
            <div style="display:flex;align-items:center;gap:.8rem;margin-bottom:.6rem">
              <div style="flex:1;text-align:right">
                ...
                <span style="font-family:'JetBrains Mono',monospace;font-size:.72rem;{"color:"+ca+";font-weight:700" if win_a else "color:#5a5a80"}">{va}</span>
              </div>
              ...
              <div style="flex:1">
                ...
                <span style="font-family:'JetBrains Mono',monospace;font-size:.72rem;{"color:"+cb+";font-weight:700" if win_b else "color:#5a5a80"}">{vb}</span>
              </div>
            </div>""",unsafe_allow_html=True)

        st.markdown("<div style='height:.8rem'></div>",unsafe_allow_html=True)
        v1,v2=st.columns(2)
        for col,mn,wins,ow,col_c in [(v1,m1n,wins_a,wins_b,ca),(v2,m2n,wins_b,wins_a,cb)]:
            icon="🏆" if wins>ow else "🤝" if wins==ow else "🥈"
            is_w=wins>ow
            with col:
                st.markdown(f"""<div class="glass-card" style="text-align:center;border-color:{col_c if is_w else "#1f1f38"}"><div style="font-size:2rem">{icon}</div><div style="font-family:'Outfit',sans-serif;font-weight:800;font-size:1.1rem;color:{col_c}">{mn}</div><div style="font-family:'JetBrains Mono',monospace;font-size:.8rem;color:#5a5a80">{wins} dimension wins</div>{"<div class='glow-chip chip-green' style='margin-top:.5rem;font-size:.8rem'>WINNER 🏆</div>" if is_w else ""}</div>""",unsafe_allow_html=True)
        ...
        specs=[("Context",f"{ma.get('context',0)//1000}K",f"{mb.get('context',0)//1000}K"),
        ...
        st.dataframe(pd.DataFrame(specs,columns=["Spec",ma["name"],mb["name"]]),use_container_width=True,hide_index=True)
...
with tabs[6]:
    ...
    if tl_data:
        ...
        for m in tl_data:
            c=col_(models.index(m))
            st.markdown(f"""<div style="display:flex;align-items:center;gap:1rem;padding:.6rem 0;border-bottom:1px solid var(--border)"><span class="timeline-dot" style="background:{c}"></span><span style="font-family:'JetBrains Mono',monospace;font-size:.8rem;color:#5a5a80;min-width:90px">{m["release_date"]}</span><span style="font-family:'Outfit',sans-serif;font-weight:700;color:{c}">{m["name"]}</span><span style="font-family:'JetBrains Mono',monospace;font-size:.72rem;color:#5a5a80">{m.get("provider","")}</span><span class="glow-chip chip-purple" style="margin-left:auto">{m.get("overall",0)}</span></div>""",unsafe_allow_html=True)
...
with tabs[7]:
    ...
    with dd1:
        ...
        <div class="glass-card" style="border-color:{dc}">
          ...
          <div class="tag-row">
            ...
            <span class="glow-chip chip-{"green" if dm.get("open_source") else "yellow"}">{dm.get("license","")}</span>
          </div>
    ...
    with dd2:
        ...
        for d,l in zip(DIMS,DIM_LABELS):
            val=dm.get(d,0)
            st.markdown(f"""<div style="margin-bottom:.5rem"><div style="display:flex;justify-content:space-between;margin-bottom:.15rem"><span style="font-family:'JetBrains Mono',monospace;font-size:.7rem;color:#5a5a80">{l}</span><span style="font-family:'Outfit',sans-serif;font-weight:700;color:{dc}">{val}</span></div><div class="rank-bar"><div class="rank-bar-fill" style="width:{val}%;background:{dc}"></div></div></div>""",unsafe_allow_html=True)
    with dd3:
        st.markdown('<p class="section-label">SPECS</p>',unsafe_allow_html=True)
        specs_dd=[("Overall Score",str(dm.get("overall","—"))),("Context",f"{dm.get('context',0)//1000}K tokens"),
                  ("Input Cost",f"${dm.get('price_in',0):.2f}/1M"),("Output Cost",f"${dm.get('price_out',0):.2f}/1M"),
                  ("Release Date",dm.get("release_date","—")),("MMLU",f"{dm.get('mmlu',0):.1f}%" if dm.get("mmlu") else "—"),
        ...
        for label,val in specs_dd:
            st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:.35rem 0;border-bottom:1px solid var(--border)"><span style="font-family:'JetBrains Mono',monospace;font-size:.7rem;color:#5a5a80">{label}</span><span style="font-family:'Outfit',sans-serif;font-weight:700;font-size:.85rem;color:#eeeef8">{val}</span></div>""",unsafe_allow_html=True)

    if len(models)>1:
        st.markdown('<p class="section-label" style="margin-top:1.2rem">VS. FIELD RADAR</p>',unsafe_allow_html=True)
        st.plotly_chart(make_radar([dm]+[m for m in sorted_models[:4] if m["name"]!=dm["name"]]),
            use_container_width=True,config={"displayModeBar":False})

    if dm.get("public_resp"):
        st.markdown('<p class="section-label" style="margin-top:1rem">SAMPLE RESPONSE</p>',unsafe_allow_html=True)
        st.markdown(f"""<div style="background:var(--s1);border:1px solid var(--border);border-radius:12px;padding:1rem;font-family:'JetBrains Mono',monospace;font-size:.82rem;color:#c8c8e8;line-height:1.7;white-space:pre-wrap">{dm["public_resp"]}</div>""",unsafe_allow_html=True)
...
with tabs[8]:
    ...
    st.dataframe(df_full.style.background_gradient(subset=num_cols,cmap="Blues"),
    ...
    st.dataframe(df_full[num_cols+["MMLU %","GSM8K %","HumanEval %"]].describe().round(2),use_container_width=True)
